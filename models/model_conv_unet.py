...
class ModelConvUnet(nn.Module):
    def __init__(self, nFilters = 32, nBottleneck = 512, context_size = 64, predictor_size = 32):
        ...
        self.down1 = nn.Sequential(
            ...
            BlockDown(nFilters, nFilters, kernel_size=3, stride=2) #32
        )

        self.down2 = nn.Sequential(
            BlockDown(nFilters, nFilters * 2, kernel_size=3, stride=1),
            BlockDown(nFilters * 2, nFilters * 2, kernel_size=3, stride=2)#16
        )

        self.down3 = nn.Sequential(
            BlockDown(nFilters * 2, nFilters * 4, kernel_size=3, stride=1),
            BlockDown(nFilters * 4, nFilters * 4, kernel_size=3, stride=2) #8
        )

        self.down4 = nn.Sequential(
            BlockDown(nFilters * 4, nFilters * 8, kernel_size=3, stride=1),
            BlockDown(nFilters * 8, nFilters * 8, kernel_size=3, stride=2) #4
        )

        self.down5 = BlockDown(nFilters * 8, nBottleneck, kernel_size=3, stride=1)#4
        
        
        #decoder
        
        self.up1 = NewBlockUp(nBottleneck, nFilters * 4) #8
        self.up2 = NewBlockUp(2*(nFilters * 4), nFilters * 2) #16
        self.up3 = NewBlockUp(2*(nFilters * 2), nFilters) #32
        self.up4 = nn.ConvTranspose2d(2*nFilters, out_channels, kernel_size = 4, stride = 2, padding=1) #>64
        self.tanh = nn.Tanh()
