...
#TODO idm increase workers maybe
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"train loss, {loss_train:.3f}, val loss, {loss_val:.3f}", file=open(f"/scratch/results_evc/{args.run_name}_epoch_mse_convUnet_8x8.csv",'a'))
        print(f"Epoch: {epoch}\n", file=open(f"/scratch/results_evc/{args.run_name}_batch_mse_convUnet_8x8.csv", 'a'))
    #     wandb.log({
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (features, target) in pbar:
        ...
        # })
        print(args.run_name)
        print(f"loss, {losses.val:.3f}, loss_avg, {losses.avg:.3f}", file=open(f"/scratch/results_evc/{args.run_name}_batch_mse_convUnet_8x8.csv", 'a'))
